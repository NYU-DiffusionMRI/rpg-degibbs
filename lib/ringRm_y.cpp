...
//     fftw_complex *data_abs = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * n *numlines);
...
//     fftw_complex *sh3 = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * n *(2*nsh+1));
    
//     for (int j = 0; j < n*numlines; j++) {
//         data_abs[j][0] = sqrt(pow(data[j][0],2) + pow(data[j][1],2));
//         data_abs[j][1] = 0;
//     }
...
//         fftw_execute_dft(p,&(data_abs[n*k]),sh3);
...
//         for (int j = 0; j < 2*nsh+1; j++)
//         {
//             fftw_execute_dft(pinv,&(sh[j*n]),&sh3[j*n]);
//         }
